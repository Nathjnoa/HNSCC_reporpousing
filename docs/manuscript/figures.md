...
Ten paired tumour / adjacent-normal specimens were profiled by data-independent acquisition
(DIA) mass spectrometry (MaxQuant), yielding 3,352 quantified proteins. Differential abundance
(limma, paired contrast; |log₂FC| > 1, FDR < 0.05) defined the tumour proteomic signature, which
was mapped to candidate drugs across four resources (DGIdb, ChEMBL, Open Targets, L2S2). Targets
were placed in a tumour protein–protein interaction (PPI) network (STRING), partitioned into
...
## Figure 2. The HNSCC tumour proteome is broadly and coherently remodelled.

(**A**) Volcano plot of differential protein abundance in tumour versus adjacent-normal tissue
(limma paired contrast; n = 10 pairs). Of 3,352 quantified proteins, 666 were differentially
abundant (FDR < 0.05, |log₂FC| > 1): 329 over- and 337 under-abundant in tumour. Labels mark
proteins with |log₂FC| > 3.5 and FDR < 0.01 plus a fixed set of mechanistically relevant targets
(e.g., EGFR, proteasome and Complex I subunits, DNMT1); labelling is illustrative and does not
...
samples, colour encodes z-scored abundance. (**C**) Hallmark gene-set enrichment analysis (GSEA,
...
(NES); the oxidative-phosphorylation signature is among the strongest down-regulated programmes
(NES ≈ −2.2).
...
(3,513 → 458 → top-ranked → LOD-stable) is provided as Figure S1.

## Figure 4. Network modules organize targets into druggable mechanistic axes.

(**A**) Tumour PPI network (STRING) laid out by a stress algorithm with intra-module weighting,
coloured by Louvain module and **data-driven drugability tier**: *approved* modules (≥ 1 approved
drug targeting a module node; 11 modules, individual colours), *hubs-only* modules (0 approved but
≥ 2 druggable hubs; slate), and *below-threshold* modules (grey). Grey modules still contain
druggable hubs (36/99 of the total) and prioritized targets (e.g., DNMT1 → decitabine in M17);
colour marks the mechanistic axes, not exclusivity of targets. (**B**) Representative enriched
...
(DGIdb excluded for edge anchoring owing to noise); *tier* = hub-central (target is a network hub)
...
concordance: log₂FC from the DIA proteome (tumour vs adjacent-normal, n = 10 pairs) against
CPTAC-HNSCC TMT proteome (Huang et al., 2021; paired limma, 116 tumour / 66 normal, 66 pairs);
Pearson r = 0.789 over n = 636 shared genes, 86.3% directional concordance; four pillar genes
labelled. (**B**) Proteome-versus-transcriptome concordance: DIA versus TCGA-HNSC RNA-seq
(STAR-Counts, DESeq2; 520 tumour / 44 normal); r = 0.601 over n = 663 shared genes, 76.2%
directional concordance; four pillar genes labelled. Greyscale in A/B encodes background
...
0.001) alongside a composite-score bar decomposed into TargetPriority and DrugViability, ordered
by descending composite. CPTAC: 12/14 with data (11 concordant, 10 FDR < 0.05); TCGA: 14/14 (11
concordant, 11 FDR < 0.05). Colour encodes therapeutic pillar (EGFR / Proteasome / Epigenetic /
...
top-ranked and LOD-stable sets, complementing Figure 3.
